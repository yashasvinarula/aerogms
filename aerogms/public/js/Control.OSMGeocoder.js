...
L.Control.OSMGeocoder = L.Control.extend({
	...
	_geocode : function (event) {
		...
		{
			var msg = this._isLatLon(q);
			console.log("LatLon: "+msg[1]+" "+msg[2]);
			//msg = {lon, lat}
			this.options.callback.call(this, this._createSearchResult(msg[1],msg[2]));
			return;
		}
		else if (this._isLatLon_decMin(q) != null)
		{
			var msg = this._isLatLon_decMin(q);
			//msg: [ns, lat dec, lat min, ew, lon dec, lon min]
			var temp  = new Array();
			temp['n'] = 1;
			temp['s'] = -1;
			temp['e'] = 1;
			...
			this.options.callback.call(this,this._createSearchResult(
				temp[msg[1]]*(Number(msg[2]) + msg[3]/60),
				temp[msg[4]]*(Number(msg[5]) + msg[6]/60)
			));
			return;
		}
